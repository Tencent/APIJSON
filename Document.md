...
3.GET、HEAD请求是开放请求，可任意组合任意嵌套。其它请求为受限制的安全/私密请求，对应的 metho, tag, version, structure 都必须和 后端Request表中所指定的 一一对应，否则请求将不被通过。version 不传或为 null 会使用最高版本，传了有效值则会使用最接近它的最低版本。下同。<br >
