...
------------ | ------------ | ------------
 ...
 判断是否存在 | "key}{@":{<br /> &nbsp;&nbsp; "range": "ANY", <br /> &nbsp;&nbsp; "from":"Table",<br /> &nbsp;&nbsp; "Table":{ ... }<br />}<br />其中：<br />}{ 表示 EXISTS；<br /> key 用来标识是哪个判断；<br /> @ 后面是 子查询 对象，具体见下方 子查询 的说明。 | ["id}{@":{<br /> &nbsp;&nbsp; "range": "ANY",<br /> &nbsp;&nbsp; "from":"Comment",<br /> &nbsp;&nbsp; "Comment":{<br /> &nbsp;&nbsp;  &nbsp;&nbsp; "momentId":15 <br /> &nbsp;&nbsp; }<br />}](http://apijson.cn:8080/get/{"User":{"id}{@":{"range":"ANY","from":"Comment","Comment":{"momentId":15}}}})<br /> WHERE EXISTS ANY(SELECT * FROM Comment WHERE momentId=15)
