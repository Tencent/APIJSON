...
------------ | ------------ | ------------
 ...
 远程调用函数 | "key()":"函数表达式"，函数表达式为 function(Type0:value0,Type1:value1...)。函数参数类型为Object或泛型时可省略类型，即 Object:value 改写为 value | ["isNoPraise()": "isArrayEmpty(praiseUserIdList)"](http://apijson.cn:8080/get/{"Moment":{"id":301,"isNoPraise()":"isArrayEmpty(praiseUserIdList)"}})，请求完成后会调用 boolean isArrayEmpty(JSONObject request, String array) 函数，然后变为 "isNoPraise":false 这种
