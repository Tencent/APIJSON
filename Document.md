...
------------ | ------------ | ------------
 ...
 比较运算 | >, <, >=, <= 比较运算符。<br />主要用来实现子查询相关比较运算，顺便提供 <br />"id{}":"<=80000" <br />这种条件范围的简化写法。<br /> 不支持 "key=":Object 和 "key!=":Object 这两种写法，直接用更简单的 "key":Object 和 "key!":Object 替代。 |  ① ["id<=":90000](http://apijson.cn:8080/head/{"User":{"id<=":90000}})，对应SQL是`id<=90000`，即id满足id<=90000<br /><br /> ② ["id>@":{<br /> &nbsp;&nbsp; "from":"Comment",<br /> &nbsp;&nbsp; "Comment":{<br /> &nbsp;&nbsp;  &nbsp;&nbsp; "@column":"min(userId)" <br /> &nbsp;&nbsp; }<br />}](http://apijson.cn:8080/get/{"User":{"id>@":{"from":"Comment","Comment":{"@column":"min(userId)"}}}})<br /> WHERE id>(SELECT min(userId) FROM Comment)
