...
------------ | ------------ | ------------
 ...
 "key<\>":Object  =>  "key<\>":[Object]，key对应值的类型必须为JSONArray，Object为JSONObject,JSONArray之外的类型 | 包含选项范围 | "friendIdList<\>":38710，查询friendIdList包含38710的Object。一般用于查询一个数组。请求[{"[]":{"User":{"friendIdList<\>":38710}}}](http://139.196.140.118:8080/get/{"[]":{"count":3,"User":{"friendIdList<\>":38710}}})会返回一个User数组，例如上面那个
 ...
 "key$":"SQL搜索表达式"  =>  "key$":["SQL搜索表达式"]，任意SQL搜索表达式字符串，如 %key%, %k%e%y% 等 | 模糊搜索 | "name$":"%m%"，搜索包含m的名字。一般用于查询一个数组。请求 [{"[]":{"User":{"name$":"%m%"}}}](http://139.196.140.118:8080/get/%7B%22%5B%5D%22%3A%7B%22User%22%3A%7B%22name%24%22%3A%22%2525m%2525%22%7D%2C%22count%22%3A3%2C%22page%22%3A0%7D%7D) 会返回name包含"m"的User数组
