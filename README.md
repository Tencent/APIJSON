...
------------ | ------------ | ------------
 ...
 "@key":任意Object | @key为JSONObject中的关键字，作用各不相同，但都不作为查询匹配条件 | ① "@columns":"id,sex,name"，只查询id,sex,name这几列并且请求结果也按照这个顺序；<br /> ② 从pictureList获取第0张图片作为头像；<br />{<br /> &nbsp; "pictureList":["url0","url1"],<br /> &nbsp; "@position":0, //这里@position为自定义关键字<br /> &nbsp; "head()":"get(Collection:pictureList,Object:@position)"<br />}<br /> ...
